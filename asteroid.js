...
const AU_TO_M = 1.496e11; // AU to meters
const DAY_TO_S = 86400; // Day to seconds
...
// Planetary data with realistic masses (in kg)
const planetData = {
    sun: { mass: 1.989e30, radius: 7, realRadius: 696000 }, // km
    mercury: { mass: 3.285e23, radius: 0.5, realRadius: 2439.7 },
    venus: { mass: 4.867e24, radius: 1.2, realRadius: 6051.8 },
    earth: { mass: 5.972e24, radius: 1.4, realRadius: 6371 },
    mars: { mass: 6.39e23, radius: 1, realRadius: 3389.5 },
    jupiter: { mass: 1.898e27, radius: 4, realRadius: 69911 },
    saturn: { mass: 5.683e26, radius: 5.5, realRadius: 58232 },
    uranus: { mass: 8.681e25, radius: 4.5, realRadius: 25362 },
    neptune: { mass: 1.024e26, radius: 4.3, realRadius: 24622 }
};

const orbitalParams = {
    mercury: { a: 0.387 * AU_SCALE, e: 0.206, i: 7.00 * Math.PI / 180, period: 88, realPeriod: 87.97 },
    venus: { a: 0.723 * AU_SCALE, e: 0.007, i: 3.39 * Math.PI / 180, period: 225, realPeriod: 224.70 },
    earth: { a: 1.0 * AU_SCALE, e: 0.017, i: 0, period: 365, realPeriod: 365.256 },
    mars: { a: 1.524 * AU_SCALE, e: 0.093, i: 1.85 * Math.PI / 180, period: 687, realPeriod: 686.98 },
    jupiter: { a: 5.203 * AU_SCALE, e: 0.049, i: 1.30 * Math.PI / 180, period: 4333, realPeriod: 4332.59 },
    saturn: { a: 9.537 * AU_SCALE, e: 0.056, i: 2.49 * Math.PI / 180, period: 10759, realPeriod: 10759.22 },
    uranus: { a: 19.191 * AU_SCALE, e: 0.047, i: 0.77 * Math.PI / 180, period: 30687, realPeriod: 30688.5 },
    neptune: { a: 30.068 * AU_SCALE, e: 0.009, i: 1.77 * Math.PI / 180, period: 60190, realPeriod: 60182 }
};
...
function init() {
    ...
    // Initialize celestial bodies array for gravity calculations
    celestialBodies = [
...
function setupControls() {
    ...
    if (asteroidScaleSlider) {
        asteroidScaleSlider.addEventListener('input', (e) => {
            ...
            document.getElementById('asteroid-scale-value').textContent = `${asteroidVisualScale.toFixed(0)}x`;
...
function updateAsteroidScales() {
    asteroidMeshes.forEach(asteroid => {
        const baseScale = asteroid.userData.originalScale;
...
function createAsteroidBelt(asteroidData) {
    asteroidData.forEach((data) => {
        ...
        // Make asteroids clearly visible - much larger base size
        const actualDiameterKm = data.diameter;
        const size = 2 + Math.log(actualDiameterKm + 1) * 2; // Increased significantly
        const geometry = new THREE.SphereGeometry(size, 8, 8);

        // Apply vertex displacement for irregularity
        ...
        // Estimate mass based on diameter (spherical, rocky density ~3000 kg/m³)
        const radiusKm = actualDiameterKm / 2;
        const radiusM = radiusKm * 1000;
        const volume = (4/3) * Math.PI * Math.pow(radiusM, 3);
        asteroid.userData.mass = volume * 3000; // Rocky composition
        asteroid.userData.actualDiameter = actualDiameterKm;

        // Store original size for scale adjustments
        asteroid.userData.originalScale = size;
        asteroid.scale.setScalar(asteroidVisualScale);

        // Set initial position and velocity using Keplerian elements
...
function initializeAsteroidPosition(asteroidMesh) {
    ...
    // Solve Kepler's equation (simplified - using mean anomaly as eccentric anomaly)
...
function initializeAsteroidVelocity(asteroidMesh) {
    ...
    if (r < 1e6) return; // Avoid issues with very small distances
    
    const mu = G * planetData.sun.mass;
    const v_mag = Math.sqrt(mu * (2/r - 1/a_m));
    
    // Velocity is perpendicular to radius in orbital plane
    ...
    // Convert to simulation units (AU_SCALE per day)
...
function calculateGravitationalForce(body1, body2) {
    ...
    const distance = r.length() * (AU_TO_M / AU_SCALE); // Convert to meters
    
    if (distance < 1e6) return new THREE.Vector3(0, 0, 0); // Avoid singularity
    ...
    // Convert back to simulation units (AU_SCALE per day²)
...
function updateAsteroidWithGravity(asteroidMesh, dt) {
    ...
    if (integrator === 'verlet') {
        // Velocity Verlet integration - symplectic and energy-conserving
...
function drawAsteroidOrbit(data) {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        const E = solveKeplerEquation(M, override.e !== undefined ? override.e : data.e);
        const a = override.a !== undefined ? override.a : data.a;
        const e = override.e !== undefined ? override.e : data.e;
...
document.getElementById('close-btn').addEventListener('click', () => {
    ...
    document.getElementById('orbital-panel').classList.remove('open');
...
function setupOrbitalControls(data) {
    ...
    const updateOrbit = () => {
        ...
        // Earth's orbit: 0.983 AU (perihelion) to 1.017 AU (aphelion)
        ...
        drawAsteroidOrbit(asteroidOverrides.get(data.pdes));
    };
...
function convertSimPositionToLatLong(impactVector, earthPosition) {
    // Get local impact point relative to Earth center
...
function checkEarthCollision(asteroidMesh) {
    ...
    // Calculate impact threshold based on Earth's actual size in simulation units
    // Earth radius in simulation: 1.4 units (visual), convert to physical check
    const earthRadiusSimUnits = 1.4;
    const asteroidRadiusSimUnits = asteroidMesh.userData.originalScale * asteroidVisualScale;
    ...
    if (distance < impactThreshold) {
        ...
        const impactCoords = convertSimPositionToLatLong(asteroidPosition, earthPosition);
        
        // Calculate impact velocity (km/s)
        ...
        zoomToImpact(earthPosition, asteroidPosition);
    } else {
        const warningElement = document.getElementById('orbital-warning');
        if (warningElement && asteroidMeshes.selected === asteroidMesh) {
            // Convert distance to AU for meaningful comparison
            const distanceAU = distance / AU_SCALE;
            if (distanceAU < 0.05) { // Within 0.05 AU of Earth
                warningElement.textContent = `⚠ CLOSE APPROACH: ${(distanceAU * AU_TO_M / 1e6).toFixed(2)} million km`;
                warningElement.style.borderColor = 'rgba(244, 67, 54, 0.5)';
            }
        }
    }
}

function zoomToImpact(earthPosition, impactPosition) {
    const startPosition = camera.position.clone();
    const impactDirection = new THREE.Vector3().subVectors(impactPosition, earthPosition).normalize();
    const finalCameraPosition = earthPosition.clone().add(impactDirection.multiplyScalar(10));
    ...
    function animateZoom() {
        ...
        camera.position.lerpVectors(startPosition, finalCameraPosition, alpha);
...
function animate() {
    ...
    if (!isPaused) {
        ...
        // Update planets using Keplerian motion
        ...
        // Update asteroids with gravity or Keplerian motion
        asteroidMeshes.forEach(asteroid => {
            if (gravityEnabled) {
        ...
        // Planet rotations (scientifically scaled)
        sun.rotation.y += 0.001;
        mercury.rotation.y += 0.0017; // 58.6 days
        venus.rotation.y -= 0.00041; // 243 days (retrograde)
        earth.rotation.y += 0.01; // 24 hours
        mars.rotation.y += 0.0097; // 24.6 hours
        jupiter.rotation.y += 0.024; // 9.9 hours
        saturn.rotation.y += 0.023; // 10.7 hours
        uranus.rotation.y += 0.014; // 17.2 hours
        neptune.rotation.y += 0.015; // 16.1 hours
    }  
    controls.update();
